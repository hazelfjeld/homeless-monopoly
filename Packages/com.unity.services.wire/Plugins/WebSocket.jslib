...
var LibraryWebSocket = {
	...
	$webSocketInvoke: function(callback) {
		...
		var activeWasmTable = null;

		if (typeof wasmTable !== 'undefined')
			activeWasmTable = wasmTable;
		else if (typeof Module !== 'undefined' && Module['wasmTable'])
			activeWasmTable = Module['wasmTable'];

		if (activeWasmTable && typeof activeWasmTable.get === 'function') {
			activeWasmTable.get(callback).apply(null, args);
			return;
		}

		throw new Error('WebSocket callback table is not available.');
	...
	WebSocketConnect: function(instanceId) {
		...
		instance.ws.onopen = function() {
			...
			if (webSocketState.onOpen)
				webSocketInvoke(webSocketState.onOpen, instanceId);
		...
		instance.ws.onmessage = function(ev) {
			...
			if (ev.data instanceof ArrayBuffer) {
				...
				try {
					webSocketInvoke(webSocketState.onMessage, instanceId, buffer, dataBuffer.length);
				} finally {
					_free(buffer);
				}
		...
		instance.ws.onerror = function(ev) {
			...
			if (webSocketState.onError) {
				...
				try {
				    webSocketInvoke(webSocketState.onError, instanceId, msgBuffer);
				} finally {
					_free(msgBuffer);
				}
		...
		instance.ws.onclose = function(ev) {
			...
			if (webSocketState.onClose)
				webSocketInvoke(webSocketState.onClose, instanceId, ev.code);
...
autoAddDeps(LibraryWebSocket, '$webSocketInvoke');
